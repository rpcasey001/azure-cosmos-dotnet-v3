...
#pragma warning restore SA1401 // Fields should be private
        ...
        internal static Result WriteOperation(ref RowWriter writer, TypeArgument typeArg, ItemBatchOperation operation)
        {
            ...
            if (operation.RequestOptions != null)
            {
                ...
                if (options.IndexingDirective.HasValue)
                {
                    ...
                    switch (options.IndexingDirective.Value)
                    {
                        case IndexingDirective.Default:
                            indexingDirectiveString = IndexingDirectiveStrings.Default;
                            break;
                        case IndexingDirective.Include:
                            indexingDirectiveString = IndexingDirectiveStrings.Include;
                            break;
                        case IndexingDirective.Exclude:
                            indexingDirectiveString = IndexingDirectiveStrings.Exclude;
                            break;
                        default:
                            throw new InvalidEnumArgumentException(nameof(options.IndexingDirective), (int)options.IndexingDirective.Value, typeof(IndexingDirective));
                    }
        ...
        internal int ApproximateSerializedLength()
        {
            int length = 0;
        ...
        internal virtual async Task MaterializeResourceAsync(CosmosJsonSerializer serializer, CancellationToken cancellationToken)
        {
            ...
            {
                this.body = await BatchExecUtils.StreamToMemoryAsync(this.ResourceStream, BatchConstants.MaxResourceSizeInBytes, cancellationToken);
            }
        }
